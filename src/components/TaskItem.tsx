...
export const TaskItem = React.memo<TaskItemProps>(({ task }) => {
  ...
  const handleUpdate = useCallback(async () => {
    if (editText.trim() && editText !== task.text) {
      try {
        await updateTaskText({ id: task.id, text: editText.trim() }).unwrap();
    ...
    setIsEditing(false);
  }, [editText, task.text, task.id, updateTaskText, dispatch]);
  ...
  const handleToggleComplete = useCallback(
    ...
    [task.id, completeTask, incompleteTask, dispatch]
  );
